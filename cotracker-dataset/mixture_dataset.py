...
from .omni3d_object_dataset import Omni3DDataset
from .omni3d_object_dataset_lowtexture import Omni3DDatasetLowtexture
from .kubric_movif_dataset import KubricMovifDataset
import os
os.environ["OPENCV_IO_ENABLE_OPENEXR"]="1"
class MixtureDataset(CoTrackerDataset):
    def __init__(
        self,
        data_root,
        crop_size=(384, 512),
        ...
        self.omni3d_dataset=Omni3DDataset(
            data_root=os.path.join(data_root, "OpenXD-OmniObject3D-New/views-omiobject3d/"),
            crop_size=crop_size,
            seq_len=seq_len,
            traj_per_sample=traj_per_sample,
            sample_vis_1st_frame=sample_vis_1st_frame,
        ...
        self.kubric_movif_dataset=KubricMovifDataset(
            data_root=os.path.join(data_root, "kubric_movi_f"),
            crop_size=crop_size,
            seq_len=seq_len,
            traj_per_sample=traj_per_sample,
            sample_vis_1st_frame=sample_vis_1st_frame,
        ...
        self.omni3d_dataset_lowtexture=Omni3DDatasetLowtexture(
            data_root=os.path.join(data_root, "OpenXD-OmniObject3D-New/lowtexture/"),
            crop_size=crop_size,
            seq_len=seq_len,
            traj_per_sample=traj_per_sample,
            sample_vis_1st_frame=sample_vis_1st_frame,
        ...
        # print('Using Mixture Dataset,with {} {}'.format(len(self.omni3d_dataset),len(self.kubric_movif_dataset)))

    def getitem_helper(self, index):
        # import pdb;pdb.set_trace()
        dataset_no=np.random.randint(100)
        if dataset_no>=0 and dataset_no<=25:
            return self.omni3d_dataset.getitem_helper(index)
        elif dataset_no>25 and dataset_no<70:
            return self.omni3d_dataset_lowtexture.getitem_helper(index)
        else:
            index=np.random.randint(len(self.kubric_movif_dataset))
            return self.kubric_movif_dataset.getitem_helper(index)

    def __len__(self):
        return 10000#len(self.omni3d_dataset)+len(self.kubric_movif_dataset)
