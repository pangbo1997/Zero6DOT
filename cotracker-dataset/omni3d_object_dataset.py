...
class Omni3DDataset(CoTrackerDataset):
    def __init__(
        self,
        data_root,
        crop_size=(384, 512),
        ...
        super(Omni3DDataset, self).__init__(
            data_root=data_root,
            crop_size=crop_size,
            seq_len=seq_len,
    ...
    def prepare_track_from_depth(self,start_ind,seq_name):
        data_base_dir=os.path.join(self.data_root, seq_name)
        ...
        # rot90=np.array([1,0,0,0,0,-1,0,1,0]).reshape(3,3)
        # import pdb;pdb.set_trace()
        ...
        choose_idx=np.random.permutation(pts_0.shape[0])[:2000]
        pts_0=pts_0[choose_idx]
        
        pts_0_homo=np.concatenate((pts_0,np.ones(pts_0.shape[0])[:,None]),axis=1)
        
        u_corrd,v_corrd,z=get_uvcoord(pts_0,K)
        track=np.concatenate((u_corrd[:,None],v_corrd[:,None]),axis=1)
        ...
        vis_list.append(vis)
        for idx in range(start_ind+1,start_ind+self.seq_len):
            ...
            u_corrd,v_corrd,z=get_uvcoord(pts,K)
            track=np.concatenate((u_corrd[:,None],v_corrd[:,None]),axis=1)
            # import pdb;pdb.set_trace()
            ...
            #hist,bins = np.histogram(delta_z,bins=200)
            vis=delta_z<bins[1]
            ...
            # import pdb;pdb.set_trace()
        # import pdb;pdb.set_trace()
        return np.stack(track_list)[:,:,[1,0]],np.stack(vis_list)
    ...
    def getitem_helper(self, index):
        ...
        while not find:
            # import pdb;pdb.set_trace()
            try:
                index=np.random.choice(np.arange(len(self.seq_names)))
                gotit = True
                ...
                img_paths = os.listdir(rgb_path)
                rgbs = []
                for i, img_path in enumerate(img_paths):
                    rgb=imageio.v2.imread(os.path.join(rgb_path, f'{i}.png'))
                    # rgb[rgb==1]=0
                    rgbs.append(rgb)

                rgbs = np.stack(rgbs)[:,:,:,:3]
                # import pdb;pdb.set_trace()
                # random crop
                ...
                traj_2d,visibility=self.prepare_track_from_depth(start_ind,seq_name)
                find=True
            except:
                print('Failed, keep finding...')
