...
from cotracker.models.core.cotracker.ekf import *
...
class CoTracker(nn.Module):
    def __init__(
        ...
        self.rot_state_gru=nn.GRU(input_size=128,hidden_size=128,batch_first=True)
        self.rot_predictor=nn.Linear(128,4)

        self.tran_state_gru=nn.GRU(input_size=128,hidden_size=128,batch_first=True)
        self.tran_predictor=nn.Linear(128,3)

        with torch.no_grad():
            self.rot_predictor.weight.fill_(0)
            self.rot_predictor.bias.fill_(0)
            self.rot_predictor.bias[0]=1

            self.tran_predictor.weight.fill_(0)
            self.tran_predictor.bias.fill_(0)
    ...
    def forward_iteration(
        ...
        rot_state,_=self.rot_state_gru(ffeats.mean(dim=2))
        delta_rots=F.normalize(self.rot_predictor(rot_state),dim=-1)

        tran_state,_=self.tran_state_gru(ffeats.mean(dim=2))
        delta_trans=self.tran_predictor(tran_state)

        # pts=self.pts_predictor(ffeats.mean(dim=(1,2)))
        tracks=coord_predictions[-1]
        vis_all=vis_e
        pred_pose_list=[]
        update_pose_list=[]
        ...
        with torch.no_grad():
            x=torch.zeros(B, 7+3*pts.shape[1]).to(ffeats.device)
            x[:,:3]=init_pose[:,0,:3,3]
            x[:,3:7]=matrix_to_quaternion(init_pose[:,0,:3,:3])
            x[:,7:]=pts.reshape(B,-1)
            x_cov=torch.zeros(B, 7+3*pts.shape[1],7+3*pts.shape[1]).to(ffeats.device)
            
        eps=1e-8

        for i in range(S):
            delta_tran=delta_trans[:,i]
            delta_rot=delta_rots[:,i]

            x,x_cov=ekf_predict(x,x_cov,delta_tran,delta_rot)
            pred_pose_list.append(x[:,:7])  


            track=tracks[:,i]
            with torch.no_grad():
                pose=torch.eye(4,device=x.device)[None].repeat(B,1,1)
                pose[:,:3,:3]=quaternion_to_matrix(x[:,3:7].detach())
                pose[:,:3,3]=x[:,:3].detach()
            x,x_cov=ekf_update(x,x_cov,pose,track,K)
            update_pose_list.append(x[:,:7])
        ...
        # import pdb;pdb.set_trace()
        return coord_predictions, vis_e, feat_init,pred_pose_list, update_pose_list

    def forward(self, rgbs, queries, iters=4,pts=None,init_pose=None,K=None, feat_init=None, is_train=False):
        ...
        pred_pose=torch.zeros((B,T,7), device=device)
        update_pose=torch.zeros((B,T,7), device=device)

        ind_array = torch.arange(T, device=device)
        ind_array = ind_array[None, :, None].repeat(B, 1, N)

        ind_array2 = first_positive_inds[:,None,:].repeat(1,T,1)
        track_mask2=((ind_array2-ind_array)>=-30) #& ((ind_array-ind_array2)<15) 

        track_mask = (ind_array >= first_positive_inds[:, None, :])#.unsqueeze(-1)

        track_mask=(track_mask & track_mask2).unsqueeze(-1)
        ...
        while ind < T - self.S // 2:
            ...
            if prev_wind_idx > 0:
                ...
                # vis_local=torch.sigmoid(vis_init_[:,-1])[None].repeat(1,8,1,1)
                # coords_init_=coords_init_[vis_local[:,:,:,0]>0.9].reshape(1,8,-1,2)
                # vis_init=vis_local


            # import pdb;pdb.set_trace()
            coords, vis, __, pred_pose_list, update_pose_list= self.forward_iteration(
                fmaps=fmaps,
                ...
                iters=iters,
                pts=pts_[:,:wind_idx],
                init_pose=init_pose[:,ind : ind + self.S],
                K=K
            )

            # pose=torch.eye(4,device=pts.device)[None].repeat(B,1,1)
            # pose[:,:3,:3]=quaternion_to_matrix(update_pose_list[self.S//2][:,3:7].detach())
            # pose[:,:3,3]=update_pose_list[self.S//2][:,:3].detach()
            # init_pose=pose[]
            
            # if is_train:
            #     vis_local=torch.sigmoid(vis[:, :S_local])
            #     vis_predictions.append(vis_local)
            #     for coord in coords:
            #         coord_temp=[]
            #         for i in range(S_local):
            #             coord_temp.append(coord[:,i][vis[:,i]>0.9][None,None])
            #         coord_predictions.append(torch.cat(coord_temp,dim=1))
            ...
            if is_train:
                ...
                coord_predictions.append([coord[:, :S_local] for coord in coords])
                pred_poses.append(pred_pose_list)
                update_poses.append(update_pose_list)
                wind_inds.append(wind_idx)

            traj_e[:, ind : ind + self.S, :wind_idx] = coords[-1][:, :S_local]
            vis_e[:, ind : ind + self.S, :wind_idx] = vis[:, :S_local]
            pred_pose[:,ind : ind + self.S]=pred_pose_list[-1][:,:S_local]
            update_pose[:,ind:ind+self.S]=update_pose_list[-1][:,:S_local]
        ...
        train_data = (
            (vis_predictions, coord_predictions, wind_inds, sort_inds,pred_poses,update_poses)
            if is_train
            else None
        )
