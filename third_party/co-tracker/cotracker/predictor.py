...
class CoTrackerPredictor(torch.nn.Module):
    ...
    def forward(
        ...
        if queries is None and grid_size == 0:
            ...
            tracks, visibilities,ffeats = self._compute_sparse_tracks(
                video,
                queries,
                segm_mask,
                grid_size,
        ...
        return tracks, visibilities,ffeats
    ...
    def _compute_sparse_tracks(
        ...
        tracks, __, visibilities, __, ffeats = self.model(rgbs=video, queries=queries, iters=6)
